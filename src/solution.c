...
static int decode_nmearmc(char **val, int n, sol_t *sol)
{
    ...
    pos2ecef(pos,sol->rr);
    sol->stat= (mode=='D') ? (unsigned char)SOLQ_DGPS : (unsigned char)SOLQ_SINGLE;
    sol->ns=0;
...
static int decode_nmeagga(char **val, int n, sol_t *sol)
{
    ...
    if (sol->time.time==0) { //LONG INT DOUBLE
    ...
    pos2ecef(pos,sol->rr);
    sol->stat=0<=solq&&solq<=8?(unsigned char)solq_nmea[solq]:(unsigned char)SOLQ_NONE;
    sol->ns=(unsigned char)nrcv;
...
static void readsolopt(FILE *fp, solopt_t *opt)
{
    ...
    for (i=0;fgets(buff,(int)sizeof(buff),fp)&&i<100;i++) { /* only 100 lines */
                   ...
                   const solopt_t *opt)
{
    ...
    if (opt->timef) time2str(time,s,timeu);
        ...
        if (86400*7-gpst<0.5/pow(10.0,(double)timeu)) {
            week++;
            gpst=0.0;
        }
