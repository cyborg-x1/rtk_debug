...
static int decode_xf5raw(raw_t *raw)
{
    ...
    rcvTimeScaleCorr = (unsigned char)I1(p+26);
    ...
    for (i=0,p+=27;(i<nsat) && (n<MAXOBS); i++,p+=30) {
        ...
        sys = (unsigned char)(U1(p)==1)?SYS_GLO:((U1(p)==2)?SYS_GPS:((U1(p)==4)?SYS_SBS:SYS_NONE));
        prn = U1(p+1);
...
static int decode_gpsephem(int sat, raw_t *raw)
{
    ...
    eph.sva    = uraindex((double)I2(&puiTmp[126]));
